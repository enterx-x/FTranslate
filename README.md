...
3. 点击 `生成双语 PDF`。
4. 生成完成后，左侧阅读器会自动切换到 `左右双语`，英文原文在左、中文译文在右，缩放、页码和滚动会同步。
...
Kimi K2.5 属于非思考模型，应用会强制使用 `temperature=0.6`、`top_p=0.95` 并传入 `thinking disabled`；只有模型名明确包含 thinking 的 Kimi K2 模型才会使用 `temperature=1` 和思考模式。应用使用私有 `pdf2zh` 运行时时会自动补丁其 OpenAI-compatible 翻译器，并通过环境变量传入 Kimi 需要的温度、思考模式、超时和重试参数；OpenAI、DeepSeek 和 Custom provider 的 PDF 翻译默认仍使用确定性的 `temperature=0`。
...
同一份 PDF 即使后来被新建成新的论文记录，只要文件大小和修改时间对应的 `sourceHash` 一致，也会优先复用本机已有双语 PDF 缓存，并把缓存元数据重新绑定到当前论文记录，避免重复启动 PDFMathTranslate。
...
- Univer 内置条件格式和数据验证插件已启用，可通过表格右键菜单或内置工具栏设置“根据条件填色”和单元格下拉选项
- 右键菜单里的 `AI 填充选区`
- 行高和自定义列会随本地工作簿一起保存
...
研究表格还提供 `AI 大观分析`：选择若干已绑定论文的行后，AI 会读取这些行的表格信息、论文元信息、已有笔记、AI 缓存和可用 PDF 上下文，批量提炼该领域反复出现但仍未解决的核心缺口，并且只提出 1 个 idea。OpenAI provider 会通过 Responses `web_search_preview` 做实时网页查新；其它 provider 会尝试拉取 Semantic Scholar 公开检索结果作为额外上下文。提示词会明确要求给出科学问题、可验证技术路线、实验方案、评价指标和失败诊断，同时拒绝模块堆砌、改名蹭热点和空泛口号。分析进度和历史结果会写入本机 `localStorage`，返回页面后仍能看到上次的运行进度、失败原因、最终结果和历史记录；如果长时间没有更新，会标记为中断，方便重新执行。
...
src/
  ...
  renderer/
    ...
    lib/
      papers.ts          论文库记录
      pdfViewportSync.ts 左右 PDF 阅读器滚动比例同步
