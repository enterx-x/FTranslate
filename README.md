...
## 2026-07-18 COMET-MBR 论文级翻译选优（0.1.38）

- arXiv 标题与完整摘要现在使用 HY-MT2 固定生成 3 份论文级候选；公式、引用、实验数值、方法名、截断、英文回声和上下文泄漏先经过硬门禁，再由独立的 `Unbabel/wmt22-comet-da` 做整篇 COMET-MBR 选优。最终只选一份完整候选，不跨候选拼句，也不调用聊天式 AI 润色。
- 候选生成与 COMET 采用顺序调度：先生成候选并释放 HY-MT2，再启动外置 COMET CPU worker，避免在本机 15 GB 内存 / 8 GB 显存上同时常驻两个大模型。COMET 环境位于 `E:\FTranslateTools\comet-mbr`，模型不进入安装包；运行中心支持检测和显式安装，缺失或失败时会显示真实降级原因并回退到通过结构校验的单候选或 NLLB/Argos。
- SQLite 缓存升级到 `v11/mbr-v1`，保存候选数量、选中 seed、评估器和降级原因。共享批次失败后会按论文隔离重试，单篇坏样本不会拖垮同批其他论文；三份 HY 候选全部失败时才逐篇尝试后备引擎。
- 卡片内状态条显示真实的候选生成、候选校验、质量评估、降级与完成阶段，并保留实际耗时；状态使用完整边框、图标和克制色阶，不再用高噪声侧边色条。
- 30 条、10 个领域的真实 7B 基准中，单候选与 MBR 均实现硬门禁、受保护内容和禁用误译 100%；MBR 必需术语召回为 `99.17%`，参考译文 COMET 从 `0.84889` 提升到 `0.85593`（`+0.00704`），7/10 个领域上升。唯一暴露的 `terminal set -> 终止集` 已追加窄范围标准化并单独回归；独立人工盲评尚未完成，因此不把自动指标写成最终人工验收通过。
- COMET worker 实测：探针 `8.689 s`，冷态 6 对评分 `12.584 s`，暖态 `2.622–2.786 s`，峰值工作集约 `5.24 GB`，卸载 `446 ms`，退出 `126 ms`，无孤儿进程。
- 完整发布验证通过：107 个测试文件通过、1 个跳过，746 项测试通过、3 项跳过；两套 TypeScript、Vite renderer、Electron main、win-unpacked 与 NSIS 均构建成功。源码版和安装包内 arXiv 专项视觉门禁均通过，人工复核候选生成、COMET 完成态、1366px 三列结果和高级筛选，未发现重叠、遮挡、截断或横向溢出。
- Windows 安装包为 `dist/PDF Translation Reader Setup 0.1.38.exe`（157,152,943 bytes，SHA-256 `5E9D3B71436504465ECE462BCB1134F39EB997DA4BF1ECB2A4829A3384FA1646`）。包内确认携带 COMET worker 与隔离安装脚本，但不携带模型；已从最新 `win-unpacked` 启动独立热预览（PID 29976、CDP 9458、profile `.tmp-visual-check/hot-preview-0.1.38-20260718-174439`）。

## 2026-07-17 arXiv 官方相关性检索（0.1.38）

- 普通搜索默认直接请求 arXiv 官方 `sortBy=relevance`，实时结果、分页、正常缓存和过期缓存均保持官方 ID 顺序；旧的“本页相关排序”运行时值自动迁移为官方相关性，不再对当前页二次打分、重排或误删。
- 中文科研查询继续采用“不同概念 `AND`、同一概念别名 `OR`”。`人形触觉`、`灵巧手 + 触觉 + 机器人操作`、`动力学模型 + MPC + 机器人`均有独立必需概念组；默认界面可展开查看实际发送给 arXiv 的完整结构化表达式。
- 桌面端和移动端排序入口统一为“相关性（arXiv）/提交时间/更新时间”；相关性不显示无意义的升降序控件，“最新论文”仍固定为提交时间降序。检索缓存升级到 `title-abstract-v8`，并包含实际表达式与 `concept-groups-v1` 构建器身份。
- 新增 20 条中英文科研检索基准和可断点续跑的官方 API 脚本。真实 20/20 查询规则初评为 P@10 `0.990`、nDCG@10 `0.923`、Top-10 跑题率 `0.010`、平均 API 延迟约 `859 ms`；该结果基于确定性概念规则，尚未冒充人工盲评结论。
- COMET-MBR 翻译链路、30 条自动基准、完整构建、源码/安装包视觉检查、NSIS 安装包和隔离热预览均已完成；当前仅保留独立人工盲评尚未审签这一质量风险。

## 2026-07-17 论文级上下文学术翻译（0.1.37）

- arXiv 标题不再脱离论文语境单独翻译：标题携带摘要开头，摘要分段携带论文标题和最近英文前文；背景只用于消歧，当前段仍是唯一允许输出的内容。标题上下文最多 700 字符，摘要上下文最多 900 字符，避免挤占 7B 模型的正文窗口。
- HY-MT2 继续使用专用机器翻译而非聊天式 AI 润色。每个段落在生成前接收当前段术语、论文上下文和学术书面语约束；公式/引用数字标记必须原样且恰好出现一次。上下文泄漏、异常扩写、重复助词或重叠谓语会触发一次不同 seed 的严格本地重译，仍不合格才进入 NLLB/Argos 降级链。
- 批量翻译身份从“仅原文”改为“原文 + 上下文 + 风格”，相同句子属于不同论文时不会误共享语境；只有三者完全一致才去重。SQLite 翻译缓存升级到 v10，并记录 `paper-context-v1`、HY-MT2 模型身份和 `academic-en-zh-v3` 术语表，只淘汰旧译文，不修改收藏、已读、阅读队列、项目或笔记。
- 通用术语层补充 `locomotion tracking -> 运动跟踪`。真实 HY-MT2 7B CUDA 门禁覆盖强化学习、人形机器人、PINN/PDE、CBF/MPC、触觉/灵巧操作、RoboMamba/RoboCasa/ManiSkill/MetaWorld 和受保护数字标记；六句冷启动约 5.8-7.0 秒，模型常驻后的同批热态约 1.7-1.8 秒，三项真实集成测试全部通过。
- 状态栏和设置页只把既有 7B badge 更新为“HY-MT2 7B 上下文质量档”，没有增加常驻开关或新卡片。源码版完整构建通过 104 个测试文件、709 项测试；Settings/arXiv 专项视觉检查通过，1366px 三列结果、详情栏、模型路径和新 badge 无重叠、截断或横向溢出。
- Windows 安装包为 `dist/PDF Translation Reader Setup 0.1.37.exe`（157,128,589 bytes，149.85 MiB，SHA-256 `75FA0B8734471BD7065E0027C7BBA9C891AA5D609E9A7F90F4BEC85EB41DE8B8`）。安装版 Settings/arXiv 视觉检查与人工复核通过；确认产物后只删除 0.1.36 安装器和 blockmap，并从最新 `win-unpacked` 启动隔离热预览（PID 37120、CDP 9457）。
...
- 中文组合查询改为“确定性概念直接映射 + 仅剩余未知概念调用本地查询翻译”。`人形触觉`仍完全走确定性术语，不会再次出现 doll 误译；`强化学习 故障诊断`则同时保留 reinforcement learning，并补入 fault diagnosis，不再静默丢掉后半个概念。
- 起止年份填反时自动规范为有效区间，并让正序/反序输入共享同一缓存身份；检索缓存升级到 `title-abstract-v7`，旧错误布尔表达式生成的结果自动失效。
- 修复渐进翻译的跨进程断点：快速中文标题此前在 Electron IPC 校验层被丢弃，完整摘要链路会重复翻译标题；现在字段会安全透传，并可直接补全“已有中文摘要、缺少中文标题”的 SQLite 部分缓存，后续读取即时复用完整结果。
- 定向覆盖查询表达式、混合中文回退、年份归一化、IPC 字段透传、部分缓存升级与翻译会话，共 8 个文件、151 项 arXiv 专项测试通过；完整构建为 102 个测试文件、672 项测试全部通过。
- 源码版与安装包内 arXiv 视觉回归均通过，覆盖翻译阶段反馈、详情栏收起/展开、备选论文浮层、单/双/三列及 1366/1440/1920px，无遮挡或横向溢出。Windows 安装包为 `dist/PDF Translation Reader Setup 0.1.34.exe`（157,108,990 bytes，SHA-256 `D859F9D2D86D84CAE52B62AFBCD1B0A1E96A5D401BA343F5625EEEEA04E21AE4`）；旧 0.1.33 安装器与 blockmap 已清理。

## 2026-07-16 PDF 图表提取吞吐与状态修复（0.1.33）

- 修复长论文图表工作台“候选已经出现、却长期 0 张可用”的根因：普通方法图、结果图和表格不再等待 PDF.js 整页 operator list 与原生图片物化；只有未手调的实验装置/案例照片才启用内嵌图片几何辅助，最终图片仍来自页面高分辨率渲染。
- 高清提取不再先把整页按 2.25 倍栅格化再裁一小块。现在按同页候选的安全扩展区域做局部页面渲染，保持相同输出清晰度、矢量文字和坐标轴，同时显著减少像素工作量与 canvas 峰值。
- 提取进行中，尚未生成的卡片和筛选项明确显示“等待提取/待提取”，完成后才归入“待调整”，避免把正常排队误报成裁剪失败。
- 真实 25 页论文、21 个候选回归：正常 GPU 的首帧专项由 149.5 秒时仍只有 10/21，降到 29.9 秒内完成 21/21；完整场景 36.8 秒通过，PDF 首屏 931 ms，工作台无横向溢出，关闭后 PDF canvas 仍有真实像素。前三张导出图与裁剪编辑器已人工复核，无坐标偏移、文字截断或图片降质。
- 完整构建通过 102 个测试文件、667 项测试；安装包内图表专项再次通过，PDF 首屏 1,983 ms、最终 21/21。Windows 安装包为 `dist/PDF Translation Reader Setup 0.1.33.exe`（157,107,679 bytes，SHA-256 `172D4D86948E2FBB09C785B9A7D84FDA1EC702ECE1C1463D06A36B3E2ECF6EF6`）；旧 0.1.32 安装器、blockmap 与本轮临时目录已清理。
...
- 修复 PDFMathTranslate 生成页中的科研图变红、变黑，以及引用附近绿色/红色框错位的问题。生成后会从原文恢复被重写的图像/Form XObject，并只在中文重排页移除坐标已经失效的 `/Link` 注释；旧本地缓存首次打开时也会自动校正一次，不重新调用 AI、不增加翻译 token。
- 论文库批量操作栏新增“批量生成中文 PDF”，支持 1/2/3 并发、总进度、逐篇页数/百分比、排队/缓存/失败状态、详情收起和失败项重试。批量任务只回传元数据，不把多份大型 PDF base64 同时送回 renderer；首次运行环境安装加互斥保护，避免并发任务争抢同一 Python venv。
- 原来的纯文字翻译状态替换为可访问的进度条：支持确定百分比、页数、准备阶段的非确定动画、完成/缓存/失败状态与减少动画模式。AI 助手的主模板按钮改为浅色高对比选中态，右侧模板列表允许两行说明并消除卡片重叠。
- 真实 39 页用户论文副本验证：恢复 17 个图像对象、移除中文页 802 个错位链接框；第 3 页彩色科研图在中文单语和 dual 产物中均恢复原色。源码构建为 102 个测试文件、661 项测试通过；PDF 专项首屏为 3,000 ms，适宽居中、中文模式、左右双语、进度条和图表提取视觉门禁通过，AI 模板选中态对比度为 10.36/12.86，论文库 1366/1440/1920px 与批量栏无横向溢出。
- Windows 安装包为 `dist/PDF Translation Reader Setup 0.1.31.exe`（157,106,176 bytes，SHA-256 `7F5455A5D8FC71C78AC8FDFD565CCA7B4B0D53CCAAA9A89457B65C84FE46D71C`）。构建后已删除 0.1.30 的旧安装器与 blockmap，并从 `dist/win-unpacked` 启动可见热预览确认主窗口正常响应。
