...
扫描件、拍照 PDF 或已把字体转成轮廓的 PDF 没有浏览器可提取的文字层。移动阅读器检测到这种文件后不再停在错误提示，而是显示“识别并生成双语”：PDF.js 在当前设备逐页渲染受控尺寸的 JPEG，再直接发送到用户配置的、支持图片输入的 OpenAI 兼容模型；模型一次返回按阅读顺序排列的原文段落和中文译文。识别结果和完成页码逐页写入当前浏览器，可以停止后继续，退出再打开也不会重做已成功页面。页面图片不会上传到 FTranslate 或 Vercel，但会发送到用户自己选择的模型服务商；该路径会按页调用接口，使用前应确认模型支持图片输入及相应计费。
