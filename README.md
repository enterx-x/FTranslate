...
手机版会在写入前校验 PDF 文件头与 PDF.js 文档结构，单个 PDF 上限为 64 MB；arXiv 下载支持取消并在 45 秒后超时。网页会申请浏览器持久存储，并把导入/arXiv 下载的原 PDF 以原始 `ArrayBuffer` 写入独立 IndexedDB，避免 iPhone Safari 无法持久化 Blob URL；OCR 原文、逐段译文、阅读进度和论文索引也按论文写入本地存储。少数刚收录的 arXiv 摘要可能暂时还没有对应 PDF；源站返回 404 时页面会明确提示“PDF 暂未开放”，可稍后重试或选择另一篇。重复保存同一份 PDF 会保留阅读进度；如果源文件内容或 arXiv 版本发生变化，会从第 1 页重新开始并使旧段落译文失效，避免原文与译文串版。任何 PDF 导入论文库后都会立即在当前设备后台逐页 OCR 全文，并把每页英文原文先保存到本机；这一阶段不会读取 API Key、不会调用 DeepSeek/OpenAI-compatible 接口，也不会生成中文。退出阅读页、切换到原始 PDF 或切换底部入口不会取消后台 OCR。论文库会显示等待、进度、完成或失败状态。OCR 全部完成后，用户进入“连续双语”并明确点击“翻译全文”，系统才按原页顺序翻译，页内逐段生成，完成一页就保存一页；可中途停止并保留已完成部分。正文采用 17–19px、自然左对齐的紧凑小说式排版，不使用卡片、段落边框或“译文/重译”标签切断阅读；向下阅读会自动隐藏标题栏、模式栏和处理状态，向上滑动或返回顶部即可恢复操作区。更换 Base URL 或模型后，“翻译剩余”会更新旧配置生成的译文。

扫描件、拍照 PDF 或已把字体转成轮廓的 PDF 没有浏览器可提取的文字层。移动版对所有导入 PDF 统一使用 PDF.js 在当前设备逐页渲染受控尺寸页面，再由随应用提供的 Tesseract.js 英文 OCR 提取并重排原文，所以不依赖 PDF 自带文字层。DeepSeek 不需要支持图片输入，页面图片也不会发送到 DeepSeek、FTranslate 或 Vercel；只有用户在 OCR 全文完成后点击“翻译全文”，识别出的纯文字才会进入翻译接口。OCR 版面结果如果退化成“一词一段”或把一行误作一个段落，阅读器会对照整页文本恢复完整自然段；孤立的短大写标签、纯页码和被裁断的短小写片段不会进入正文或翻译队列，跨碎片的行尾连字符会重新连接为同一个单词。旧版本若残留“识别完成”但没有保存任何段落，新版会自动忽略这个无效状态并从第 1 页重新识别；若旧缓存只剩后面的页面，则从第一个缺页开始补齐，不再把“最后处理页”误当作前页都已保存。每一页识别出的全部原文会整页写入当前浏览器并记录已处理页集合，因此没有识别到正文的空白页也不会被反复处理。同一论文的写入严格串行，避免前页被后续写入覆盖。阅读器在论文库与原 PDF 之间切换时保持挂载、正文和滚动位置，整页刷新后也会从 IndexedDB 恢复已成功页面。公网网页首次使用会从同一 FTranslate 站点加载约 7 MB 的 OCR worker、英文数据和 WASM 核心，之后由浏览器缓存复用。
