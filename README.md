...
- PDF 打开链路不再用逐字符回调构造 `Uint8Array`。同一份 15.6 MB / 25 页样本中，renderer 的 base64 解码由约 1091 ms 降到约 24 ms；论文库中的原文、翻译缓存和双语 PDF 资源也改为并行读取。
...
- 单个英文词会进入词典卡：同时显示本地中文译文、音标、词性、英文释义、例句、近义词和反义词。联网词典不可用时仍保留本地翻译和明确的降级提示。
...
使用选词翻译：打开“PDF 阅读”后直接拖选文字；卡片会自动开始翻译。只选择一个英文单词时会自动进入词典模式。扫描版 PDF 没有文字层时仍需要先 OCR。
