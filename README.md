...
- 已用用户提供的 13 篇真实论文做全页回归，共 122/122 页完成，全部直接命中 PDF 文字层，未触发 OCR；覆盖 1.55–61.58 MB、4–15 页、IEEE/arXiv、双栏、公式、长表和首页图文等版式。`T TouchDreaming 2026.4.14.pdf` 的 14/14 页也全部来自文字层，OCR 页数为 0。
- `T TouchDreaming 2026.4.14.pdf` 与此前测试的 `2604.13015v2.pdf` 大小均为 14,327,098 字节，SHA-256 均为 `E9C449299F73D0BFA5C1231C8EA469E9ACF25F83FCE57E3820ACE49A89304B15`，确认是同一个文件，不是手机上传时发生了转换。阅读器现在明确显示“PDF 文字层 N 页 / 本地 OCR N 页”，不再把通用原文缓存误写成“OCR 缓存”。
- 本地原文提取缓存版本提升为 `6`，图表缓存版本保持 `2`。部署后旧论文无需删除；应用会自动清除旧版错误原文/图表索引并重新逐页提取，已保存的原 PDF 仍保留在当前浏览器。
