...
- `T TouchDreaming 2026.4.14.pdf` 与此前测试的 `2604.13015v2.pdf` 大小均为 14,327,098 字节，SHA-256 均为 `E9C449299F73D0BFA5C1231C8EA469E9ACF25F83FCE57E3820ACE49A89304B15`，确认是同一个文件，不是手机上传时发生了转换。iPhone 截图随后证明旧生产版本仍把前两页降级到了本地 OCR，根因范围收敛为 Safari 中 PDF.js 文字项的数组/字符串/坐标对象兼容差异，而不是文件内容。
- 手机文字层读取不再依赖 `items.flatMap()`、原生字符串 `.trim()` 或可迭代坐标数组；每个文字项会独立做安全转换，单个异常项不能让整页降级。标准结构重排失败时，只要原始文字项仍可读，就先走“Safari 兼容重排”并继续标记为 PDF 文字层；只有文字项为空或正文确实不足时才启动 OCR。页面来源同时区分“结构化文字层 / 兼容重排 / 本地 OCR”，并保留实际降级原因。
- 本地原文提取缓存版本提升为 `7`，图表缓存版本保持 `2`。部署后旧论文无需删除或重传；应用会清除旧版错误原文/图表索引并重新逐页提取，已保存的原 PDF 仍保留在当前浏览器。重建时会按相同 `sourceHash` 尝试恢复仍匹配的已有译文；原文已经改变的错误 OCR 译文不会错误套用到新文字层段落。
- 本轮修复已部署到固定生产地址 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)。iPhone Safari 如果仍显示旧界面，先正常刷新一次；重新打开已有论文时会自动按新版本重建原文与图表缓存。
