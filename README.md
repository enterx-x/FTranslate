...
- 已用三篇用户提供的真实论文做全页回归：`T-BAL_CoP-ESP32_2025.12.24.pdf`（7 页）、`T-DATA_HumanoidVTA_2025.10.28.pdf`（4 页）和 `T-HRI_SGR_2025.3.5.pdf`（8 页）。共 19 页均直接命中 PDF 文字层并完整结束，没有触发 OCR 兜底；另用 14 页 Touch Dreaming 论文覆盖首页图文、双栏正文、长表和末页收尾。
- 本地原文提取缓存版本提升为 `5`，图表缓存版本提升为 `2`。部署后旧论文无需删除；应用会自动清除旧版错误原文/图表索引并重新逐页提取，已保存的原 PDF 仍保留在当前浏览器。
