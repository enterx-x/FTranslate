...
- 扫描页先以最长边约 2100px 快速识别；只有整体置信度低于质量门、没有留下有效段落或过滤后损失过大的页面，才自动用约 2600px PNG 精扫。普通清晰扫描页少处理约 35% 的像素，疑难页仍保留原高精度兜底。
- 导入或保存 arXiv PDF 时，会先完成 PDF 与论文库索引落盘，再启动全文原文提取。网页端论文索引直接同步写入该站点的 `localStorage`，所有后续进度按顺序排队保存；即使启动时旧论文库读取稍晚返回，也会与当前已经导入的论文合并，不能再用旧空列表覆盖新论文。导入完成后页面会明确显示“PDF 已保存到当前浏览器”。
