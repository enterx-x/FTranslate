...
FTranslate 当前聚焦日常「发现论文—阅读—积累」：桌面主入口收敛为今日、论文库、arXiv 检索、PDF 阅读和设置。实验矩阵、研究表格、证据图谱与独立聊天入口退出默认导航；已有本地数据不删除。以下历史工作台规划不再作为继续增加模块的依据。

- 在「今日」填写研究兴趣、排除词、每天本地时间和最多推荐数量，保存后可立即运行一次。定时简报默认关闭，需主动开启。
- 应用运行期间按本机时间执行，错过时间后在启动时补跑；关闭应用或电脑关机时无法生成或发送通知。完成后有新论文时尝试 Windows 通知，通知是否可见受系统通知设置影响。当前没有云端定时、邮件或手机推送。
- arXiv 公开标题与摘要用于筛选，推荐不代表已阅读全文或已验证代码。AI 为可选增强，启用后使用 AI 设置中的服务，将研究兴趣、公开候选摘要及反馈信息用于规划与排序；未开启时采用规则筛选。
- 简报保存到应用 userData 下的 `daily-brief.json`，包括偏好、历史与可撤销反馈；API Key 继续由原有 AI 设置管理。下载阅读复用论文库与 PDF 阅读器，取消保存对话框不会添加空论文。
- 首版筛选最近 7 天的新论文或更新，最多推荐 1–20 篇；排除词支持逗号、顿号、分号和换行。想读/稍后看的反馈会给相关类别有限加分，不相关论文会被排除；反馈可撤销。历史保留 30 份、反馈保留 500 条，超出简报历史的反馈只保留标题。
- 在「设置 → AI 设置」配置服务商、接口、模型和 Key；更换服务商或接口域名需要填写新 Key。AI 默认关闭，失败时会明确提示并回退规则筛选；中文兴趣建议同时提供英文关键词。自动任务失败至少等待 15 分钟重试，手动仍可重试。
- 验证入口：`npm test`、`npm run typecheck`、`npm run dist`、`npm run visual:check`。安装包界面验收使用 `$env:VISUAL_CHECK_PACKAGED='1'; npm run visual:check`。本轮结果和未覆盖边界见 `PLAN.md` 与 `MULTI_AGENT_REPORT.md`。

## iPhone 全文完整性门禁与选段提问优化（2026-07-28）

- 连续阅读的文字完整性要求不再只针对标题，而是覆盖标题、正文、图注、表格标题、表头、单元格和公式说明。结构化 PDF 重排会与整页原始文字项做词元覆盖率比对；单字母变量以及 `=`、`+`、`×`、`÷`、`≤`、`≥`、`∑`、`∫` 等关键数学符号也参与完整性检查。若重排结果大面积漏字或把 `D = A + F + S` 损坏为 `D A F S`，则拒绝该结果并自动使用兼容重排，不能把“排得整齐但内容不全”当作成功。
- 移动视觉回归使用同一组真实期望文本逐块核对标题、正文、图注、后续正文和表格内容，并校验阅读顺序、全部表头、全部数据行以及 CSS 可见高度。标题或正文被截断、段落错序、图注与图片错位、表格单元格缺失、正文使用行数裁切都会直接让发布检查失败。
- 修复视觉样本自身把长标题写出 PDF 页面边界的问题。新样本按真实 PDF 方式把长标题分为两行并保持居中；这也暴露并修复了标题行被排到首段正文之后的顺序问题。人工复核 `.tmp-mobile-visual-check/03b-reader-inline-figure-390x844.png` 与 `03c-reader-structured-table-390x844.png`，标题、正文、图片、图注、后续正文和完整表格按原页顺序显示。
- 选段 AI 提问增加“解释这段 / 说明作用 / 依据与假设 / 可能局限”四个快捷问题，但快捷项只填入问题框，不会自动发送。面板会显示来源页码，并可展开核对实际发送的论文标题、页码、原文窗口和已有译文；界面预览与真正请求共用同一份载荷构造逻辑，避免“看见的上下文”和“模型收到的上下文”不一致。
- AI 回答增加一键复制和 Safari 兼容回退；复制失败会显示真实错误，临时文本域即使遇到浏览器异常也会清理。问答仍只在用户明确点击发送后调用当前设备配置的 AI 接口，回答不会自动写入论文或持久化缓存。
- 发布验证：`npm run dist` 中 89 个测试文件、606 项测试通过，TypeScript、桌面/移动构建、Electron 与 NSIS 打包通过；本地 `npm run visual:check:mobile` 和 `npm run visual:check` 通过。Windows 安装包为 148,362,116 字节，SHA-256 为 `6C32E52F86A322773092D2A763497C40DE58FC14D8786699E4DACC99A6722F86`。
- 最新网页已部署到 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)，部署编号 `dpl_GPxoac66vDtVFjsaxTDkWCPjPbLs`。入口 `assets/index-mfJHls2D.js`、问答逻辑 `assets/MobileApp-DQ92MDEq.js`、移动样式与 PDF/KaTeX 资源均返回 HTTP 200；固定生产地址的完整移动回归通过。独立请求生产 `/api/arxiv` 的三次有限重试中两次返回 HTTP 200、Atom feed 和论文条目，一次因 arXiv 上游超时返回 HTTP 503；检索链路当前可用但上游仍有瞬时波动。
- 当前机器已找不到此前真实论文语料目录 `D:\调研PDF\调研pdf\01_严格纳入_触觉模态`，所以本轮没有伪报真实语料重跑通过；新增完整性门禁由定向单元测试、自包含 PDF、浏览器逐块断言和人工截图复核覆盖。
- 2026-07-28 的 npm 安全审计新报告了工具链 25 个、生产树 9 个高危依赖链，主要来自 `electron-builder` 与 `exceljs → archiver/glob/minimatch`。这些不是本轮引入，也不影响本轮文本回归结论，但尚未在不破坏 Excel 导出和 Windows 打包的前提下安全升级，不能继续沿用旧记录中的“0 漏洞”结论。

## iPhone 选段问答稳定性修复（2026-07-24）

- 修复长段落选区无入口的问题：允许选择最长 1,600 个字符，覆盖常见完整科研自然段；超长内容仍拒绝，避免误把整页文字发送给 AI。
- 修复选区位于超长段落后半部分时上下文丢失的问题。发送给 AI 的原文窗口现在围绕实际选区截取，而不是固定只取段落开头；窗口仍限制长度，不扩大隐私暴露面。
- 修复修改问题后旧回答仍显示的问题。文本一旦变化，旧答案和旧错误会立即清除；HTTP、网络和空响应错误会明确标记为“AI 问答”，不再误报成翻译失败。
- 中文译文上下文改为直接读取本地缓存原文，不再从 KaTeX 渲染后的 DOM 反推，避免公式的 MathML 与视觉文本重复进入请求。提示词也明确把论文标题、原文和译文视为不可信的引用材料，不能执行其中伪装成指令的内容。
- 定向回归共 17 项通过；`typecheck`、移动构建和完整移动视觉检查通过。视觉脚本实际覆盖旧回答清除、HTTP 502 错误提示、原始译文上下文、关闭面板后的迟到响应和 390×844 / 430×932 无横向溢出。
- 发布验证中 `npm run dist` 的 89 个测试文件、603 项测试全部通过，桌面视觉检查和完整依赖审计通过。Windows 安装包为 148,362,027 字节，SHA-256 为 `03E1AC3BED26C0CA8200856519D5709E81DA143D6E398DB6613760B34F89D8F2`。
- 修复已部署到 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)，部署编号 `dpl_J9KjfA1UZe1znX916afxL3wkFDqF`，入口 `assets/index-BCTeN03W.js` 与问答逻辑 `assets/MobileApp-kbd7jRRs.js` 返回 HTTP 200；固定地址的完整移动回归通过。确定性视觉回归跳过实时 arXiv，但随后独立请求生产 `/api/arxiv` 返回 HTTP 200、Atom XML 和实际论文条目。
...
- 自然段中的科研定义仍保持“小说式”连续阅读，只对确定的数学片段做行内 LaTeX。TouchDreaming 一类 `D / A / F / S` 集合定义会恢复为 `D = {(o_t, A_t, F_{t:t+τ}, S_{t:t+τ})}`，不会把整段英文强行变成公式或再次拆成碎片。
- LaTeX 是只读显示层，不覆写 PDF 文字层、OCR 原文、`sourceHash` 或本地译文缓存。选词翻译继续读取原始英文段落，避免 KaTeX 生成的 MathML/HTML 重复字符进入 DeepSeek 上下文；已有论文刷新后即可使用，无需重新上传、重新 OCR 或清除 Safari 网站数据。
- 已经带有 `$...$`、`$$...$$`、`\\(...\\)` 或 `\\[...\\]` 的模型输出会直接渲染；无法可靠判断的内容继续显示原始文本，不能为了“像公式”而改错语义。超宽展示公式只在公式块内部横向滑动，不撑宽整个页面。
- 390×844 移动视觉回归已验证 OCR 段落中的集合、上下标和 `τ` 由 KaTeX 正确显示，同时保留原始 `data-source-text`、页面横向溢出为 0，选词翻译、全文翻译、退出恢复和 API Key 缓存均未回归。
- 已部署到固定地址 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)，部署编号为 `dpl_B3tSkTtMr6Yqco91EYm3cpJpqxQo`，入口资源 `assets/index-ilZs51Vt.js` 与 KaTeX 样式均返回 HTTP 200。Safari 正常刷新并重新打开已有论文即可看到新排版，无需重新 OCR。

## 移动端公式说明段落重建 v11（2026-07-19）

- 修复 PDF 文字层把上下标作为小字号偏移文字项时，连续阅读错误生成独立碎片的问题。行聚类现在使用同一候选行内的最大正文字号判断基线容差，因此 `h`、`ℓ=1`、`τ` 等上下标会回到所属公式行，同时不会把 IEEE 首字下沉或相邻正文行误并。
- 含公式的自然语言定义不再一律按“独立公式”切块。以完整英文说明句开头且含有足够科研 prose 的行会作为自然段继续拼接；真正的紧凑展示公式仍保持单独公式块。
- 已用 `T TouchDreaming 2026.4.14.pdf` 第 7 页真实坐标复现并回归：截图中的 `Objective. Let dataset...`、三个集合定义和 prediction horizon 说明恢复为一个连续段落，不再出现独立的 `ℓ=1 is the`、`is the action chunk...` 卡片。
- 另用 13 篇、共 150 页真实论文运行生产提取链，150/150 页完成、0 OCR、0 图注错位、0 可修复断词；新增定向测试同时确认 IEEE drop cap 与纯展示公式没有回归。
- 本地原文提取缓存版本提升为 `11`。已有 PDF、译文、API Key、标签和阅读进度不会删除；Safari 刷新并重新打开论文后会从本地 PDF 自动重建原文块，无需重传或清除网站数据。
- v11 已部署到固定地址 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)，当前入口资源为 `assets/index-CtScDDi1.js`。Safari 先正常刷新，再返回论文库重新打开 TouchDreaming；状态栏会自动重建本地提取缓存。

## 移动端论文表格结构化重制 v3（2026-07-19）

- 连续双语阅读不再默认把普通论文表格显示成 PDF 截图。导入阶段会直接读取 PDF 文字层的坐标，按物理行、列中心、跨行表头和行间距重建语义化 HTML 表格；单元格文字可选择，窄屏内可独立横向滑动，页面本身不会产生横向溢出。
- 表格采用“结构优先、原 PDF 兜底”：列数、表头、行完整度和数值/公式污染必须通过置信度检查。公式密集、多级表头或文字层顺序不可靠时保留原 PDF 裁切，不能用错误的整齐表格伪装成功；原始 PDF 视图始终保留用于核对。
- 已用 `T TouchDreaming 2026.4.14.pdf` 全 14 页验证。截图中的 Table I 已恢复为 5 列 11 行；Table II–V 中可可靠解析的普通表格也完成重制，相邻列间距很窄的 `Joint velocity / Base mass` 不再粘列；公式密集的 Table VI 会保守回退原图。
- 图表缓存版本提升为 `3`。已有论文、原 PDF、译文、API Key 和阅读进度不会删除；刷新并重新打开论文后会用本地保存的原 PDF 自动重建图表索引，无需清除 Safari 网站数据或重新上传。
- 开发回归会同时记录每页结构化表格的表题、表头和行数据；390×844 视觉检查验证表格具有真实 `<table>` 结构、没有 canvas 截图、可横向滚动且不撑宽页面。
- v3 已部署到固定地址 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)，当前入口资源为 `assets/index-B6iyilJl.js`。Safari 正常刷新并重新打开已有论文即可自动补做结构化表格索引；不要删除论文、重传 PDF 或清除网站数据。

## 12 篇真实论文提取回归与段落重建 v10（2026-07-18）

- 新增可重复运行的真实 PDF 语料回归：使用用户提供的 12 篇、共 136 页论文直接执行移动端生产提取链，检查末页完成、文字层/OCR 来源、正文块上限、可修复断词以及图注锚点。当前 136/136 页完成，全部直接读取 PDF 文字层，未触发 OCR 或兼容降级；共恢复 3875 个阅读块和 113 个图表区域，112 个带文字图注的区域全部匹配，剩余 1 个无文字图注的区域仍保留原图。
- 段落间距不再用单一字体高度粗略判断，而是从当前页实际行距估计自然段间隔；这修复了正文多段粘成超长块，同时不会把普通换行拆成“一行一段”。跨布局的 `differ- / ence`、`hand- / volume` 等断词会重新连接，`non-interpenetration` 等科研复合词则保留连字符。
- 首页只把居中的标题/作者区域当作 front matter，较早出现的左右栏正文继续按双栏阅读顺序排列；算法框、十进制结果、编号行内小节和无方括号参考文献增加独立边界，避免算法文本混进正文、`2) Real-Time...` 被上一段吞并或整页 References 粘成一个大块。
- 本地提取缓存版本提升为 `10`。已有论文、原 PDF、API Key 和仍能按 `sourceHash` 对上的译文不会删除；Safari 刷新并重新打开论文后，会用已保存的原 PDF 自动重建段落与图表索引，无需清网站数据或重新上传。
- 开发者可设置 `FTRANSLATE_PDF_CORPUS_DIR` 与以 `|` 分隔的 `FTRANSLATE_PDF_CORPUS_FILES`，运行 `npm run test:mobile:corpus` 复测真实语料；详细逐页诊断输出到 `.tmp-mobile-pdf-corpus-report.json`，该临时报告不提交 Git。
- v10 已部署到固定地址 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)，当前入口资源为 `assets/index-BQ23_pIJ.js`。Safari 正常刷新后重新打开已有论文即可自动重建；不要删除论文、重新上传 PDF 或清除网站数据。

## iPhone 选词翻译与稳定性加固（2026-07-18）

- 连续双语阅读现在明确支持选词翻译：在英文原文上长按（桌面浏览器可双击）并调整系统选区，随后点击浮层中的“翻译选中内容”。只允许同一个英文原文段落内的选择进入翻译，中文译文、跨段混合选择和空选区不会误触发。
- 选词请求使用独立的科研术语提示词，并同时携带论文标题、选中内容所在英文自然段及其已有中文译文；单词或短语返回当前研究语境中的规范简体中文术语，完整句子返回准确译文，不套用全文翻译的“首次英文（中文）”展示格式。
- iPhone Safari 的选区监听不再只依赖一次 `touchend`。页面会响应 `selectionchange`，因此拖动系统选区手柄后浮层会更新到新词；浮层按 `visualViewport` 计算上下位置和最大高度，在页面底部、浏览器栏变化或缩放后仍限制在可视区域内。
- 关闭浮层、切换原始 PDF、返回论文库或改选其他词时会使旧请求结果失效，迟到响应不能重新打开已关闭浮层或覆盖新词。同一模型、同一段落内重复查询同一内容会命中当前阅读会话的 100 项内存缓存，减少重复调用；刷新后不永久保存零散选词记录。
- 全文翻译增加同步运行锁，快速双击“翻译全文”不会启动两套并发请求；意外异常会释放运行状态，已完成页面保持逐页缓存。整页安全校验失败后若逐段请求也失败，状态栏会同时显示整页校验原因和真实逐段网络错误，不再只显示前一个错误。
- 翻译设置会在保存前清理首尾空格并校验 Base URL、协议和模型名；由“翻译全文/选中内容”触发设置时必须填写 API Key，避免保存空 Key 后反复弹窗。手动打开设置仍允许清空 Key 后保存，从当前设备删除记录。
- 本版本已部署到 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)，当前入口资源为 `assets/index-BZmxYODf.js`。Safari 正常刷新即可使用，无需删除论文、重传 PDF 或清除网站数据。

## 全文术语一致性与专名首次格式（2026-07-18）

- 全文翻译不再把每一页当作互不相关的独立文本。处理第 N 页时，请求会同时携带论文标题、前面最多 4 个双语自然段，以及此前页面已经确认的专有英文名词和固定中文释义；这些上下文只用于统一术语、语气、逻辑衔接和指代，不会重复输出到当前页。
- 方法名、模型名、数据集名、系统名、算法名、模块名、软件名、作者自定义概念和缩写等专有英文名词，整篇第一次出现时显示为 `English（中文释义）`，后续页面只显示相同的 `English`。普通科研概念仍采用该领域通行的规范简体中文译名，并要求全文一致，不使用口语化或随页变化的译法。
- 每页 AI 结果新增 `terminology` 术语账本。客户端只接受确实出现在当前英文原文中的新术语，并在保存页面时一并缓存；即使模型偶尔重复括注，客户端也会按账本把已介绍术语恢复为纯英文，把当前页首次术语规范为 `English（中文释义）`。
- AI 全文校对版本提升为 `2`。已有 v1 译文会显示一次“待 AI 对照”，重新处理后术语账本、前文上下文和译文均保存在当前浏览器；刷新或退出阅读器后继续生效，无需删除论文或重新上传 PDF。
- 术语一致性版本已部署到固定生产地址 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)，当前入口资源为 `assets/index-rEkEA5z0.js`。iPhone Safari 正常刷新后打开已有论文，再点击一次“AI 对照并翻译”即可按 v2 重建译文和术语账本；不要清除 Safari 网站数据。

## 双视图 AI 段落兜底（2026-07-18）

- 导入 PDF 时仍只在当前设备读取文字层或执行本地 OCR，不读取 API Key、不调用 DeepSeek，也不生成中文。只有用户明确点击“翻译全文 / AI 对照并翻译”后才开始联网处理。
- 全文翻译改为每页一次请求，并同时提供两份内容完全重合的文本视图：`blocks` 保留每段的 `index / type / text`，`continuousText` 把同页全部片段按当前顺序连续拼接。AI 用逐段视图保留已经正确的标题、公式、图注和自然段，用连续视图检查误拆、误合并与顺序异常；只输出一份去重后的按阅读顺序双语段落。
- 提示词明确要求只有存在异常时才调整边界，禁止补写、改写论文观点或把公式/图注并入正文。返回结果还要通过重复词计数、原文 token 覆盖率和总长度比例安全门；明显漏文或无关结果不会覆盖本地原文，而是保留原段落并退回逐段翻译。
- 已有译文会显示“待 AI 对照”，无需删除论文或重传 PDF；点击一次后按页写入 `aiReflowVersion=1`，退出或刷新后不会重复处理已完成页面。AI 重排即使改变图注哈希，图片仍会按 `Fig./Figure/Table + 编号` 语义锚点贴回对应图注。
- 双视图版本已部署到固定生产地址 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)，当前入口资源为 `assets/index-LtSo75eJ.js`。iPhone Safari 正常刷新后打开已有论文：旧译文会标为“待 AI 对照”，点击“AI 对照并翻译”一次即可逐页校正并缓存；不要删除论文、重传 PDF 或清除 Safari 网站数据。

## 连续双语段落与图注定位修复（2026-07-18）

- 忠实提取同时约束两类相反错误：真实自然段不能被粘成一段，同一自然段也不能因普通换行、双栏续写、连字符、连接词或 `Fig.` 缩写被拆开。新段落会依据整段左边界的首行缩进、参考文献编号、项目符号和论文行内小标题判断；`Modality Tokenizers.`、`Results and analysis.` 等小标题即使被 PDF 文字层塞进上一行，也会恢复为独立段落。
- 跨栏后的 lowercase 续写、以介词/连词结尾的断块和跨布局的复合词会重新接回原段；跨行章节标题也会保留为同一个标题。该规则不按每个句号拆段，普通多句正文、引用范围和 `Fig. 2. This result...` 仍保持原段。
- 图表不再使用剔除图内文字之前的旧正文序号。连续阅读每次显示时都会用稳定的 `captionHash` 在当前正文中重新定位：Figure 紧贴在图注上方，Table 紧贴在表题下方；正文块数量变化或旧图表缓存不会再让图片隔着多段漂移。
- 已用 `T TouchDreaming 2026.4.14.pdf` 的真实 14 页生产提取链复测。此前第 4 页图片与图注相差 7 个正文块、第 13 页表格最多相差 15 个块；修复后识别到的 13 个 Figure/Table 均与对应图注只相差 `0.25` 个排序单位。第 6 页原先粘成 2398 字的大块已按 `Modality Tokenizers / Per-Finger Encoder / Transformer Trunk / Modular Action Experts` 恢复为独立段落，同时跨栏断开的同段后半句会重新接回。
- 本地原文提取缓存版本提升为 `9`。旧论文的 PDF、元数据和 API 配置不会删除；刷新并重新打开论文后会从保留的原 PDF 重建段落与图表索引，只有 `sourceHash` 仍匹配的已有译文才会复用。
- v9 已部署到固定生产地址 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)，当前入口资源为 `assets/index-CoJJ1sTE.js`。iPhone Safari 正常刷新并重新打开旧论文即可自动重建，无需删除论文、重传 PDF 或清除网站数据。

## 连续双语原文提取质量回归（2026-07-18）

- 手机连续阅读现在使用独立的“忠实阅读提取”链路，不再复用会主动过滤作者、单位、短句和参考文献的论文分析提取器。标题、作者与机构、摘要、章节、正文、公式、图注和 References 都会保留；数字开头的作者单位按普通正文显示，不再误排成超大章节标题。
- 双栏页面会先按几何位置合并跨栏标题和 IEEE 两行表题，再按左栏到右栏重排。`TABLE I` 与下一行表名、跨行章节标题、跨行图注、断行复合词和引用范围会恢复成完整块；独立公式不会再粘进后续正文。
- 原 PDF 的图和表继续作为原图插入连续阅读流。表格单元格、坐标轴和图内标签会从正文中排除，同时保留完整图注；正文换行恰好以 `Fig. 2. This result...` 开头时不会再被误判成一张图。
- 已用用户提供的 13 篇真实论文做全页回归，共 122/122 页完成，桌面/Node 基准环境全部直接命中 PDF 文字层，未触发 OCR；覆盖 1.55–61.58 MB、4–15 页、IEEE/arXiv、双栏、公式、长表和首页图文等版式。`T TouchDreaming 2026.4.14.pdf` 的 14/14 页在该基准环境也全部来自文字层，共重排为 219 个正文块。该结果只能证明 PDF 本身具有完整文字层，不能替代 iPhone Safari 真机验证。
- `T TouchDreaming 2026.4.14.pdf` 与此前测试的 `2604.13015v2.pdf` 大小均为 14,327,098 字节，SHA-256 均为 `E9C449299F73D0BFA5C1231C8EA469E9ACF25F83FCE57E3820ACE49A89304B15`，确认是同一个文件，不是手机上传时发生了转换。v7 在同一 iPhone 上仍逐页进入 OCR，说明异常发生在文字项兼容处理之前；最终定位到 PDF.js `getTextContent()` 内部使用 `for await...of` 聚合 `ReadableStream`，旧版 Safari 没有该异步迭代接口，因而在返回文字项前抛出 `undefined is not a function`。
- 手机文字层读取现改为直接调用 PDF.js `streamTextContent()`，并通过 Safari 支持的 `getReader().read()` 逐块收集文字，不再经过出错的异步迭代聚合路径；聚合读取只保留为兼容兜底。拿到文字后仍使用安全下标、字符串转换和坐标校验；标准结构重排失败时，只要原始文字仍可读，就走“Safari 兼容重排”。只有两条文字层路径都失败或正文确实不足时才启动 OCR，页面会显示可展开的实际降级原因。
- 本地原文提取缓存版本提升为 `8`，图表缓存版本保持 `2`。部署后旧论文无需删除或重传；应用会清除 v7 错误 OCR 原文并重新逐页提取，已保存的原 PDF 仍保留在当前浏览器。重建时会按相同 `sourceHash` 尝试恢复仍匹配的已有译文；原文已经改变的错误 OCR 译文不会错误套用到新文字层段落。
- v8 流式文字层修复已部署到固定生产地址 [https://ftranslate-mobile.vercel.app](https://ftranslate-mobile.vercel.app)，当前入口资源为 `assets/index-q3pj3S91.js`。iPhone Safari 先正常刷新一次；重新打开已有论文时会自动按 v8 重建原文缓存，不要删除论文、不要重传 PDF，也不要清除 Safari 网站数据。
