...
## arXiv 与论文库列表规则（2026-07-12）

- arXiv 默认使用单列结果流，搜索结果负责扫描，右侧 Inspector 负责摘要、评分与后续操作；不再以三列卡片牺牲标题和摘要可读性。
- 论文标题最多显示两至三行，作者和次要元数据单行省略；标签必须限制宽度，英文长词、路径和 DOI 允许安全断行。
- 列表 hover 只做 1–2px 位移，选中态使用低饱和蓝紫底色；详情进入不超过 280ms，并支持减少动态效果。
