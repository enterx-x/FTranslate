...
- 全量验证：`npm run dist` 通过（84 个测试文件、494 项测试、TypeScript、桌面 renderer/Electron 和 Windows NSIS 安装包）；`npm run ios:sync` 同步最新移动资源和 3 个 Capacitor 插件；`npm audit --omit=dev --json` 为 0 个生产依赖漏洞。本地与最终生产地址的移动回归均通过。
- 公网发布：最终 Vercel 部署 `dpl_6yUFP7TcaE2TM375BWdytfWAm5Va` 已绑定 `https://ftranslate-mobile.vercel.app`。带缓存参数 `?v=20260715-ocr-persistence-compact-final` 返回 200，并通过真实 arXiv 检索/PDF 入库、OCR 中途切换与退出恢复、三页完成后整页刷新恢复 6 个双语块、紧凑字号、改名标签和删除闭环。
...
| 2026-07-15 | 小说式正文过大，OCR 前页在切换或退出后消失 | 阅读器退出即卸载；逐段覆盖缓存缺少按论文串行保护；Safari 滚动锚定把视口推向后页 | 字号降至 17–19px；阅读器会话常驻；OCR 整页批量保存并串行写入；三页处理中切换/退出及整页刷新回归均恢复第 1–3 页 | 部署后用原 14 页论文继续处理，确认不清除网站数据即可看到此前真正落盘的页面；旧版本从未成功写入的缺失页需重新 OCR |
