...
## 2026-07-12 arXiv / 论文库视觉治理

- 已完成：arXiv 从三列卡片改为单列科研结果流，保留筛选、翻译、阅读队列与详情 Inspector；论文库统一导航、列表、进度、批量操作和 Inspector 的层级。
- 溢出治理：长论文标题、作者、标签、路径、项目关系和状态查询均具备 clamp、ellipsis 或 `overflow-wrap:anywhere`。
- 动效：加入结果行 hover、选中态、详情进入、按钮按压；减少动态效果和减少透明度偏好均可降级。
- 视觉门禁同步改为验证 1366/1440/1920 下单列结果流，而不再强制旧三列卡片。
